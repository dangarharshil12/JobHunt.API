...
namespace JobHunt.Services.JobSeekerAPI.Repository
{
    public class ResumeRespository : IResumeRepository
    {
        ...
        {
            var localPath = Path.Combine(_webHostEnvironment.ContentRootPath, "Resumes", $"{Guid.NewGuid()}{resume.FileName}{resume.FileExtension}");

            using var stream = new FileStream(localPath, FileMode.Create);
            await file.CopyToAsync(stream);

            var httpRequest = _httpContextAccessor.HttpContext.Request;
            var urlPath = $"{httpRequest.Scheme}://{httpRequest.Host}{httpRequest.PathBase}/Resumes/{Guid.NewGuid()}{resume.FileName}{resume.FileExtension}";
